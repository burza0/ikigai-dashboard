...
// Real locations in Kraków with accurate coordinates
const locations = ref([
  {
    id: 1,
...
const initializeMap = async () => {
  try {
    ...
    if (GOOGLE_MAPS_API_KEY === 'twoj_nowy_klucz' || GOOGLE_MAPS_API_KEY.includes('DEMO')) {
      throw new Error('🔑 Proszę zastąpić placeholder w .env prawdziwym kluczem Google Maps API')
    }
    
    console.log('🗺️ Initializing Google Maps with API key:', GOOGLE_MAPS_API_KEY.substring(0, 20) + '...')
    ...
    if (error.message.includes('API key')) {
      friendlyError = '🔑 Problem z kluczem Google Maps API. Sprawdź konfigurację w pliku .env'
    } else if (error.message.includes('referer')) {
      friendlyError = '🌐 Klucz API nie jest autoryzowany dla tej domeny'
    } else if (error.message.includes('billing')) {
      friendlyError = '💳 Problem z rozliczeniami w Google Cloud Console'
    }
    
    mapError.value = friendlyError
    mapLoading.value = false
  }
}
...
const createLocationMarkers = () => {
  ...
  console.log(`Created ${markers.length} markers on Google Maps`)
}
...
const showLocationDetails = (location: any) => {
  detailsModal.value.location = location; detailsModal.value.show = true
}
